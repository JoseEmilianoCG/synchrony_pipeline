...
def write_latex_report(paths: dict, metrics_df: pd.DataFrame, opt_metrics_df: pd.DataFrame = None):
    ...
    if opt_metrics_df is not None:
        opt_metrics_df = opt_metrics_df.copy()

        opt_metrics_df.columns = [
        col.replace("%", "\\%") for col in opt_metrics_df.columns
            ]

        latex_opt_table = opt_metrics_df.to_latex(
            index=False,
            float_format="%.3f",
            escape=False  # 👈 importante
        )
    else:
        latex_opt_table = "No disponible"
...
def generate_report():
    ...
    write_latex_report(paths, metrics_df, opt_metrics_df)
